...
function getAllClients(req,res,next){
    ...
    checkUser(token1,token2).then(function(result){
        ...
        if(result == 0){
            ...
            db.dbs.any('SELECT a.cltuid, a.name AS company, a.phone, a.email, a.pic, a.sender, a.create_at, a.update_at, a.is_active, a.is_delete, a.type, b.name AS rolename, b.id FROM sms.clients AS a LEFT JOIN sms.roles AS b ON a.type = b.id WHERE a.is_delete = false  ORDER BY a.update_at desc', [page,itemperpage])
            .then(function (data) {
                if (data.length == 0) { 
                    res.status(200)
                    .json({
                    ...
                    db.dbs.any('SELECT a.cltuid, a.name AS company, a.phone, a.email, a.pic, a.sender, a.create_at, a.update_at, a.is_active, a.is_delete, a.type, b.name AS rolename, b.id FROM sms.clients AS a LEFT JOIN sms.roles AS b ON a.type = b.id WHERE a.is_delete = false ORDER BY a.update_at desc LIMIT $2 OFFSET $1 * $2', [page,itemperpage])
                    .then(function (data) {
